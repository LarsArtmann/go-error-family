...
- **BuildFlow config exists (`.buildflow.yml`, added 2026-09-15):** skips `go-auto-upgrade` (jsonv1tov2 migrator, see above), `pnpm-audit` (only lockfile is `website/`'s; Dependabot covers it), `go-structure-linter` (BuildFlow's embedded snapshot ignores the project's `.go-structure-linter.yaml`; run the CLI directly — it honors the `flat` preset and exits 0), and `branching-flow` (phantom analyzer wants phantom types on every exported string of the published protocol — a breaking change by definition — and does not honor its own ignore directives: `IsIgnored` is wired into roleak/do/panic but NOT `pkg/phantom`). `.go-structure-linter.yaml` carries the `flat` preset: the root package IS the published library API; `/pkg/` restructure would break 50+ consumers.
...
- **Website `typescript` must stay on 6.x:** `astro check` (`@astrojs/language-server`) requires TypeScript's programmatic API, which the native compiler (TS 7+) does not ship. A dep bump to `^7.0.2` (2026-09-13) broke the `website-deploy` check step on master; pinned back to `^6.0.0`. Do NOT accept TS 7 bumps until Astro officially supports the native compiler (tracked at withastro/roadmap#1321). Also: run website commands via `nix develop -c pnpm ...` — the system `node` on this machine is a Bun shim and fails on `node:sqlite`.
