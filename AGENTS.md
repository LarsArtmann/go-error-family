...
- **Website (`website/`):** Astro 7 + Starlight + Tailwind v4 documentation site. Firebase Hosting target `errorfamily` in the `lars-software` project. Domain: `errorfamily.lars.software`. Build verified: `astro check` (0 errors/warnings/hints) and `astro build` (14 pages) both pass. Automated deploys via `.github/workflows/website-deploy.yml` (triggers on `website/**` changes to master). **Requires** `FIREBASE_SERVICE_ACCOUNT_LARS_SOFTWARE` GitHub secret to be set. Manual deploy: `nix run .#deploy` from the `website/` directory. The website is NOT part of the Go workspace — it's a separate Node.js project with its own `flake.nix`.
