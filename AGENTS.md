...
**Status:** v0.10.1 released (2026-09-15): CI + Release green on origin/master, all 7 module tags proxy-indexed, pkg.go.dev serving v0.10.1. Deploy Website went red 2026-09-15→2026-09-18 (TS 7 re-bump + lockfile drift, third occurrence of the class) — fixed locally 2026-09-18 with regenerated lockfile + 8 transitive vulns cleared; goes green again on the next `website/**` push. Local: all tests pass with `-race` across root + all submodules, golangci-lint v2.13.2 = 0 issues in all 7 modules, erraudit 0 findings
...
- CI now has explicit `bridge/` test and lint steps, plus examples module test and lint steps (`working-directory: ./examples`). The examples `go test -race` step subsumes the old `go build` step (`go test` compiles everything it tests).
...
- **varnamelen** ignore-names include Go-idiomatic short names: `tc` (test case), `f` (fmt.State — Go stdlib convention), `w` (http.ResponseWriter), `ag` (agent).
- **`strTrue`/`strFalse` const blocks are duplicated by design (accepted 2026-09-22, art-dupl -t 1):** `diagnose/helpers.go` and `diagnose/git/rules_git.go` each define identical 2-line blocks. Sharing is impossible without cost: the constants are unexported, `internal/` packages cannot cross module boundaries (`diagnose/git` is a separate module), and exporting `StrTrue` to the published API for two string literals pollutes the protocol. The blocks exist to satisfy mnd; per-package private constants are the idiomatic fix. Do not "fix" this clone again.
...
- **golangci-lint pins are `v2.13.2` everywhere (ci.yml + release.yml, 2026-09-15):** fixes a version split-brain — CI's old v2.12.2 fired `recvcheck` on `Family`/`Audience` (mixed receivers required by `encoding.TextUnmarshaler`) while v2.13.2 does not, so `//nolint:recvcheck` directives were simultaneously required (CI) and "unused" (local/BuildFlow → nolintlint error → auto-removed → CI red). Do NOT re-add those directives; keep the pins in sync with the local/BuildFlow binary instead. The `exhaustruct` → `exhaustruct_v5` rename shipped 2026-09-18 (deprecation-warning follow-up): config now uses the v5 linter name + `ignore-patterns` key, verified 0 issues across all 7 modules.
- **Go directive is uniformly `go 1.26.7` (all 7 modules + go.work, re-normalized 2026-09-18):** the deliberate toolchain bump (dprint adoption, commit 64298b6) set this floor, but automated dependency churn repeatedly regresses it (observed: root/diagnose back to `go 1.26`, bridge to `go 1.26.0`). If you see mixed `go` directives, normalize to `go 1.26.7` — that is the intended floor, matching the toolchain.
- **BuildFlow passes (re-verified 2026-09-18, result cache disabled):** `buildflow --dry-run` reports 0 failed steps; skips via config: `go-auto-upgrade`, `pnpm-audit`, `go-structure-linter`, `branching-flow`, and (since 2026-09-18) `nix-hash-fix` (no vendorHash in this flake — the step's `packages.x86_64-linux.default` probe fails structurally; deterministic 42/42 failures). `checks.format` (treefmt: gofumpt/goimports/golines/nixfmt) is green.
...
- **json/v2 reverted (2026-07-23) — and the 2026-09-15 recurrence fixed at the root:** The root module previously used `encoding/json/v2` (v0.7.0) but was reverted to stdlib `encoding/json`. No `GOEXPERIMENT` required. On 2026-09-15 the imports CAME BACK: the fleet orchestrator (`projects-management-automation run --command buildflow --fix`, running continuously over all repos) invoked `go-auto-upgrade`, whose `jsonv1tov2` migrator rewrites `encoding/json` → `encoding/json/v2` whenever `GOEXPERIMENT=jsonv2` is active — and this machine exports it GLOBALLY (required by the BuildFlow repo itself). The global env masked the breakage locally (tests/lint green) while the hermetic nix derivations and CI failed to compile. Guards now in place: (1) `skip_steps: [go-auto-upgrade]` in `.buildflow.yml`; (2) a depguard deny rule in `.golangci.yml` makes any `encoding/json/v2` import a lint error; (3) nix checks + CI `GOWORK=off` build are the compile-level canaries. **CI-parity reflex: verify with `GOEXPERIMENT= go build ./...` before claiming green** — the shell env lies on this machine. Do NOT remove the go-auto-upgrade skip without deliberately re-adopting json/v2.
- **BuildFlow config exists (`.buildflow.yml`, added 2026-09-15):** skips `go-auto-upgrade` (jsonv1tov2 migrator, see above), `pnpm-audit` (runs `pnpm audit` at the repo root; the only lockfile lives in `website/`, so the step fails closed with ERR_PNPM_AUDIT_NO_LOCKFILE — verified 2026-09-18 with the result cache disabled; the cached ✔ had hidden the failure. Skipped until BuildFlow supports subdirectory lockfiles — upstream candidate. Working remediation check: `nix develop -c pnpm audit` in `website/`), `go-structure-linter` (BuildFlow's embedded snapshot ignores the project's `.go-structure-linter.yaml`; run the CLI directly — it honors the `flat` preset and exits 0), and `branching-flow` (phantom analyzer wants phantom types on every exported string of the published protocol — a breaking change by definition — and does not honor its own ignore directives: `IsIgnored` is wired into roleak/do/panic but NOT `pkg/phantom`). `.go-structure-linter.yaml` carries the `flat` preset: the root package IS the published library API; `/pkg/` restructure would break 50+ consumers.
- **Dependabot automatic security updates DO NOT WORK for website/ (2026-09-18):** there is no `npm` entry in `.github/dependabot.yml`, but Dependabot security updates auto-run for detected manifests — and its npm updater fails on the pnpm v9 lockfile + overrides (every job since 2026-09-15 errored with a suppressed `record_update_job_error`). Dependabot ALERTS still detect vulnerabilities (detection works, remediation doesn't), and BuildFlow's `pnpm-audit` cannot see the `website/` lockfile (see BuildFlow bullet). The working remediation check is `nix develop -c pnpm audit` in `website/`; fix transitives with `pnpm update --depth Infinity` (resolves across major lines within declared ranges — this cleared 8 vulns incl. devalue/fast-uri/js-yaml/svgo on 2026-09-18 without overrides).
- **Erraudit is at zero findings (2026-09-15):** all 36 findings resolved — real error handling in diagnose/git + diagnose/postgres run paths, `errors.AsType` migration in `diagnose/command.go`, context attachment in `examples/cmd/http`, and `//nolint:legacyerrors // <reason>` on deliberate unpropagatable writes (fmt.State formatting, best-effort CLI output, post-status HTTP writes, reachability-probe Closes). Note: `//nolint:legacyerrors` is erraudit's suppression syntax and does NOT trip golangci's nolintlint; golines may wrap the comment to the statement's closing line — erraudit still honors it.
- **Website (`website/`):** Astro 7 + Starlight + Tailwind v4 documentation site. Firebase Hosting target `errorfamily` in the `lars-software` project. Domain: `errorfamily.lars.software` (live, verified serving). Build verified: `astro check` (0 errors/warnings/hints) and `astro build` (15 pages) both pass. Automated deploys via `.github/workflows/website-deploy.yml` (triggers on `website/**` changes to master) — uses the `FIREBASE_SERVICE_ACCOUNT_LARS_SOFTWARE` GitHub secret (set 2026-07-26), backed by the `github-website-deploy@lars-software.iam.gserviceaccount.com` service account (role `roles/firebasehosting.admin`). The action is pinned to `FirebaseExtended/action-hosting-deploy@500ac625 # v0.11.0`. Manual deploy: `nix run .#deploy` from the `website/` directory. The website is NOT part of the Go workspace — it's a separate Node.js project with its own `flake.nix`.
- **Website `typescript` must stay on 6.x:** `astro check` (`@astrojs/language-server`) requires TypeScript's programmatic API, which the native compiler (TS 7+) does not ship. A dep bump to `^7.0.2` (2026-09-13) broke the `website-deploy` check step on master; pinned back to `^6.0.0`. The SAME bump RECURRED 2026-09-18 (fleet `go-auto-upgrade`-style churn) and took `website-deploy` down again with a manifest/lockfile specifier mismatch — third occurrence of this class. Do NOT accept TS 7 bumps until Astro officially supports the native compiler (tracked at withastro/roadmap#1321). **Lockfile-sync rule: after ANY change to `website/package.json`, run `nix develop -c pnpm install` in `website/` and commit the regenerated `pnpm-lock.yaml` in the same change — `website-deploy` installs with `--frozen-lockfile` and fails on any specifier drift.** Also: run website commands via `nix develop -c pnpm ...` — the system `node` on this machine is a Bun shim and fails on `node:sqlite`. An untracked `website/bun.lock` appears when the Bun shim touches the project; it is not tracked, do not commit it.
