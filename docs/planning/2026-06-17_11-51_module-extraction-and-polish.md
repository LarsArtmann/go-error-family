...
1-3 above plus:
4. Add `diagnose` as explicit dependency to git/postgres submodules (they import diagnose types today via root module — after extraction, they need their own require)
5. Update `go.work` to include new modules
6. Update CI with per-module test/lint steps
7. Add `Registry.Clone()` and `Registry.RegisterTemplates()` — consistency with existing patterns
8. DRY `resolveSuggestedFix` and `renderCLI` — they duplicate the same template resolution chain
9. Update all documentation (README, AGENTS.md, SKILL.md, CHANGELOG, DOMAIN_LANGUAGE) for the new structure
