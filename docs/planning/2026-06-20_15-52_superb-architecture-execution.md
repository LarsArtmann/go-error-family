...
**North Star:** Make go-error-family a **superb classification protocol**. Per the architecture decision: **adopt both** — *library layer classifies* (go-error-family), *application layer enriches* (samber/oops), *bridge composes*. Prioritization lens: **superb architecture only** — version numbers and adoption metrics are explicitly out of scope.
...
- **A2 — Severity-ordered multi-error classification.** Today `Classify` on `errors.Join` returns the *first* non-Transient sub-error (`registry.go:62-70`) — result depends on argument order, not severity. Introduce `Family.Severity() int` (total order) and pick the **worst** family. Deterministic, preserves fail-closed retry semantics.
- **A3 — Zero-allocation sentinel lookup.** `lookupSentinel` (`registry.go:144-157`) clones the *entire* sentinels map under RLock on **every** `Classify` call. `Classify` is the hottest path in the library. Replace with `atomic.Pointer[immutableMap]` copy-on-write: reads are lock-free and allocation-free; writes copy-on-write. This is the canonical Go read-heavy-map pattern.
...
- **B3 — Kill the `Compose` split brain.** `Compose` (`constructors.go:105`) is a 1-line `errors.Join` wrapper re-added in commit `af59b55`, but CHANGELOG v0.5.0 claims it was *removed*. Either finish the removal (recommended — stdlib `errors.Join` is the protocol, `Classify` already handles multi-errors) or document why it stays. Architecturally: **remove**.
...
- **F1 — Bridge as canonical composition seam.** Document "library classifies, app enriches" as the architecture; the bridge is the *only* place both meet.
