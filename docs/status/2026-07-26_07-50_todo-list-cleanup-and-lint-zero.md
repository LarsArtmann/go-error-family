...
- **What:** Created `.github/workflows/website-deploy.yml` — triggers on `website/**` or workflow file changes to master, plus `workflow_dispatch`. Builds with pnpm, runs `astro check`, deploys to Firebase Hosting via `FirebaseExtended/action-hosting-deploy`.
...
29. **Decide on website package manager** — standardize on pnpm (for `pnpm install --frozen-lockfile` in CI) or switch CI to bun.
...
2. **Should the website use pnpm or bun?** I used bun for local verification (it was available; pnpm was not), but the CI workflow uses `pnpm install --frozen-lockfile`. The `bun.lock` is gitignored, but if you develop with bun and deploy with pnpm, lockfiles can drift. What's your preference?
