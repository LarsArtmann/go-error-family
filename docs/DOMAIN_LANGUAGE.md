...
| **Classification**            | The process of determining an error's Family from its type, interfaces, or registered sentinels/classifiers                                                                          | First-match-wins precedence: Multi-error → Classified → Retryable → Sentinels → Classifiers → Transient default |
...
| **Classifier**                | A predicate function `func(error) (Family, bool)` registered via `RegisterClassifier` for dynamic error types that can't be matched by `errors.Is` identity                          | For dynamic errors (e.g. `*sqlite.Error`) that produce fresh instances per occurrence                           |
| **Classification Precedence** | The ordered list of sources checked when classifying: Multi-error → Classified interface → Retryable interface → Registered sentinels → Registered classifiers → Transient default   | First match wins                                                                                                |
...
| **Registry**                  | Injectable container for classification sentinels, classifiers, and message templates. `DefaultRegistry` is used by package-level functions; custom registries enable test isolation | `NewRegistry()`, `Clone()`, passed via `HandleConfig.Registry`                                                  |
| **WrapOnce**                  | Idempotent wrap: returns the existing `*Error` unchanged if the error chain already contains one, preventing double-wrapping at API boundaries                                       | Nil-safe; uses `errors.AsType[*Error]` for detection                                                            |
| **HTTPHandler**               | net/http middleware that classifies the error from a handler func, writes a safe JSON response (never leaks `err.Error()`), and sets the HTTP status code from Family                | Response message comes from registered templates only — no internal leakage                                     |
