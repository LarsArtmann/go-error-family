...
The root module contains four concerns: stable classification library, experimental diagnostics, experimental agent, and examples. Extracting `agent/` and `diagnose/` into their own modules unblocks v1.0 of the classification library. The change is low-risk — both packages are already self-contained with clean dependency directions.
...
The project already has `go.work` coordinating 4 modules. The problem is that the **root module is a god-module** — it mixes the stable classification library with experimental packages. The submodules (bridge, git, postgres) are clean.
...
```
errorfamily (root package)
  ↑
  ├── diagnose/ ──────→ root (errorfamily.Classify, errorfamily.Family, errorfamily.Contextual)
  │     ↑
  │     ├── agent/ ───→ diagnose (DiagnosticResult, StatusFailed) + root (Classify)
  │     ├── git/ ─────→ diagnose (CommandRunner, RuleSpec) [via root module today]
  │     └── postgres/ → diagnose (FamilyIs, CommandRunner) + root (Transient) [via root module today]
  │
  ├── bridge/ ────────→ root (Family, IsRetryable) + oops
  └── examples/ ──────→ root (NewRejection, HandleError, etc.) [leaf nodes]
```
...
3. **diagnose → root**: diagnose imports `errorfamily.Classify`, `errorfamily.Family`, `errorfamily.Contextual`, `errorfamily.Coded`. These are all stable root types. **Clean dependency.**
...
All error types (`Family`, `Error`, interfaces) live in the errorfamily package. No sentinel errors cross module boundaries. `errors.Is`/`errors.As` work through the errorfamily types. **No accessibility risk.**
...
| **Deps (prod)**    | root + diagnose                                                                            |
| **Deps (test)**    | root + diagnose                                                                            |
...
| **Deps**   | root (v0.5.0 → v1.0.0) + samber/oops            |
...
| **Deps**   | root (v1.0.0) + **diagnose (v0.1.0) — NEW**           |
...
root (v1.0)
  ...
  ├── diagnose (v0.1) → root
  │     ↑
  │     └── agent (v0.1) → diagnose + root
  │
  ├── bridge → root + oops
  ├── git → diagnose + root
  └── postgres → diagnose + root
```

**Acyclic.** All arrows point toward the root. No cycles. ✓
...
| Step | Task                                                    |
| ---- | ------------------------------------------------------- |
| 4.1  | Tag root as v1.0.0, diagnose as v0.1.0, agent as v0.1.0 |
