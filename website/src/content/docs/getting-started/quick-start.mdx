...
Every error gets classified into one of six families. Each family maps to retry decisions, exit codes, HTTP status codes, and user-facing tone.
...
| Orchestration  | no        | 70        | 500         | System (bug)   |
...
// Orchestration — internal coordination failure (bug in the program's own logic)
err := errorfamily.NewOrchestration("render.failed", "template engine returned no output")
// IsRetryable → false, ExitCode → 70, HTTPStatus → 500
```
